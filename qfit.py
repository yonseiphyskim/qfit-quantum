...
from .solvers import SolverError, get_qp_solver_class, get_qubo_solver_class
...
QUBOSolutionStats = namedtuple(
    "QUBOSolutionStats", ["threshold", "BIC", "rss", "objective_value", "weights"]
)
...
class QFitOptions:
    def __init__(self):
        ...
        # influence QP / QUBO. Provide a list of atom names, e.g. ['N', 'CA']
...
class _BaseQFit(ABC):
    def __init__(self, conformer, structure, xmap, options, reset_q=True):
        ...
        assert options.qubo_solver is not None
    ...
    def _solve_qubo(
        ...
        logger.info("Solving QUBO for %d target values with %d models",
                    len(self._target), len(self._models))
        qubo_solver_class = get_qubo_solver_class(self.options.qubo_solver)
        assert len(self._models) > 0
        solver = qubo_solver_class(self._target, self._models)
        ...
        if do_BIC_selection:
            ...
            for threshold in loop_range:
                ...
                if segment is not None:
                    k = nconfs  # for segment, we only care about the number of conformations come out of QUBO. Considering atoms penalizes this too much
                if self.options.ligand_bic:
                    k = nconfs * natoms
                BIC = n * np.log(rss / n) + k * np.log(n)
                logger.info(f"   → RSS={rss:.4e}, nconfs={nconfs}, k={k:.1f}, BIC={BIC:.4f}")
                solution = QUBOSolutionStats(
                    threshold=threshold,
                    BIC=BIC,
                    rss=rss,
                ...
                qubo_solutions.append(solution)
    ...
    def _zero_out_most_similar_conformer(self, merge=False):
        ...
        This aims to reduce the 'non-convex objective' errors we encounter during qFit-segment QUBO.
...
class QFitRotamericResidue(_BaseQFit):
    ...
    def run(self):
        ...
        # QUBO score conformer occupancy
        self.sample_b()
        self._convert()
        self._solve_qubo(
        ...
        self._update_conformers()
        self._save_intermediate(prefix="qubo_solution")
    ...
    def _sample_sidechain(self):
        ...
        iteration = 0
        logger.info("Sampling rotamers for residue %s", self.residue)
        while True:
            ...
            for chi_index in range(start_chi_index, end_chi_index):
                ...
                self._update_conformers()
                self._save_intermediate(f"sample_sidechain_iter_{iteration}_qubo")
            ...
            if increase_chi:
                start_chi_index += 1
            iteration += 1
        logger.info("Found %d conformers for %s", len(self._coor_set), self.residue)
    ...
    def find_paths(self, segment_original):
        ...
        while len(segment) > 1:
            ...
            for fragment_multiconformer in fragment_multiconformers:
                ...
                # Run QUBO in a loop, removing the most similar conformer until a solution is found
                while True:
                    # Update conformers
                    ...
                    try:
                        ...
                        # QUBO failed and we need to remove conformers that are close to each other
                        logger.debug("QUBO failed, dropping a fragment-conformer")
                        self._zero_out_most_similar_conformer()  # Remove conformer
                        self._save_intermediate(prefix="qubo_kept")
                        continue
                    else:
...
class QFitLigand(_BaseQFit):
    ...
    def run(self):
        ...
        if self.options.numConf:
            ...
            if self.num_lig_atoms <= 25:
                ...
                num_gen_conformers = 7000 # default generate 7,000 conformers for ligands that are not small
        ...
        if self.options.cryo_em_ligand:
            self.options._ligand_cardinality = 2  # pylint: disable=protected-access
        ...
        # QUBO score conformer occupancy
        logger.info("Solving QUBO within run.")
        ...
        if self.options.ligand_bic:
            self._solve_qubo(
                threshold=self.options.threshold,
                cardinality=self.options._ligand_cardinality,  # pylint: disable=protected-access
        ...
        # run symmetry mate clash detector on all conformers out of QUBO
        qubo_coor_set = self._coor_set
        final_idx_set = []
        final_coor_set = []
        final_bs = []

        for idx, conf in enumerate(qubo_coor_set):
            b = self._bs
    ...
    def rot_trans(self):
        ...
        self._save_intermediate(prefix="trans_rot_sol")
        self._convert()
