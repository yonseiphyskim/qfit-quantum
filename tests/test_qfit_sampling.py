"""
Test protein rotamer sampling qfit.qfit.QFitRotamericResidue
"""
...
class TestQfitResidueSampling(QfitProteinSyntheticDataRunner):
    ...
    def _get_qfit_options(self, map_file_name):
        options = QFitOptions()
        options.map = map_file_name
        options.label = "FWT,PHIFWT"
    ...
    def _load_qfit_inputs(self, pdb_file, options):
    ...
    def _run_sample_sidechain_3mer(self, pdb_file, mtz_file, options):
        structure, xmap = self._load_qfit_inputs(pdb_file, options)
    ...
    def _run_setup_and_sample_sidechain_3mer(self, peptide_name, high_resolution):
        ...
        options = self._get_qfit_options(fmodel_mtz)
    ...
    @pytest.mark.skip(reason="FIXME no longer finding 2 unique conformations")
    @pytest.mark.fast
    def test_sample_sidechain_3mer_phe_p21(self):
        ...
        pairs, rmsds = _get_best_rmsds(reference_confs, new_confs, 0.75)
        assert len(pairs) == 2
    ...
    def test_sample_sidechain_serine_space_group_symops(self):
        ...
        for pdb_multi, pdb_single in self._get_all_serine_monomer_crystals():
            ...
            for pdb_symm in self._iterate_symmetry_mate_models(pdb_single):
                print(pdb_symm)
                options = self._get_qfit_options(fmodel_mtz)
                structure, xmap = self._load_qfit_inputs(pdb_symm, options)
    ...
    def test_sampling_rebuilt_tripeptide_arg(self):
        self._run_sampling_rebuilt_3mer("ARG", d_min=1.2)
    ...
    def test_sampling_rebuilt_tripeptide_asp(self):
        self._run_sampling_rebuilt_3mer("ASP", d_min=1.2)

    def test_sampling_rebuilt_tripeptide_cys(self):
        self._run_sampling_rebuilt_3mer("CYS", d_min=1.5)
    ...
    def test_sampling_rebuilt_tripeptide_his(self):
        self._run_sampling_rebuilt_3mer("HIS", d_min=1.3)
    ...
    def test_sampling_rebuilt_tripeptide_trp(self):
        self._run_sampling_rebuilt_3mer("TRP", d_min=1.5)
    ...
    def test_sampling_rebuilt_tripeptide_val(self):
        self._run_sampling_rebuilt_3mer("VAL", d_min=1.5)
    ...
    def test_qfit_convert_density_runtime(self):
        ...
        with self._run_in_tmpdir():
            ...
            fmodel_mtz = self._create_fmodel(pdb_multi, high_resolution=D_MIN)
            options = self._get_qfit_options(fmodel_mtz)
            structure, xmap = self._load_qfit_inputs(pdb_single, options)
            residue = list(structure.residues)[1]
            runner = QFitRotamericResidue(residue, structure, xmap, options)
