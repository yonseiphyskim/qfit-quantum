    ...
    for chain in np.unique(water.chain):
        ...
        for i in tmp_water.resi:
            if len(tmp_water.extract("resi", i, "==").altloc) == 1:
                dist_pdb = np.linalg.norm(
                    pdb.coor - tmp_water.extract("resi", i, "==").coor, axis=1
                )
            else:
                ...
                for alt in tmp_water2.altloc:
                    value = value + alt
                    dist_pdb = np.linalg.norm(
                        pdb.coor - tmp_water2.extract("altloc", alt, "==").coor, axis=1
                    )
            water = (
                str(i)
                + ","
                + chain
                + ","
                + str(tmp_water.extract("resi", i, "==").q)
                + ","
                + str(tmp_water.extract("resi", i, "==").b)
            )  # info we want from the water molecule
            protein = (
                str(pdb.resi[dist_pdb == np.amin(dist_pdb)])
                + ","
        ...
        for i in tmp_water.resi:
            ...
            if len(tmp_water.extract("resi", i, "==").altloc) == 1:
                ...
                for near_residue, near_chain in zip(
                    pdb.resi[dist_pdb < dist], pdb.chain[dist_pdb < dist]
                ):
                    key = (
                        str(near_residue)
                        + " "
            ...
            if tmp_water.extract("resi", i, "==").q != 1:
                ...
                for near_residue, near_chain in zip(
                    ...
                    if key not in neighbors:
                        neighbors[key] = value
    with open(
        pdb_name + "_" + str(dist) + "_partialwatercloseresidue.txt", "w"
    ) as file:
        for key, value in neighbors.items():
