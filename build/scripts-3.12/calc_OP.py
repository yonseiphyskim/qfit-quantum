...
def parse_args():
    ...
    p.add_argument("data_file", type=str, help="this is the output from step one")
    ...
    p.add_argument(
        "-b", "--Bfactor", type=float, help="Mean B Factor of Heavy atoms of PDB"
    )
    args = p.parse_args()
    return args
...
def calc_S2(pdb_file, data_mat, res):
    ...
    for d in data_mat:  # for everyline
        coord1, weight1, bfac1 = get_coords(
            pdb_file, d[0], d[1], d[6]
        )  # pdb, residue, atom, chain
        coord2, weight2, bfac2 = get_coords(
            pdb_file, d[2], d[3], d[6]
        )  # pdb, residue, atom, chain
        # Check and correct some simple data consitencies
        if weight1 != weight2:
            if ((len(weight1) > 1.001) and (len(weight2) > 1.001)) and (
                abs(sum(weight1) - sum(weight2)) > 0.0001
            ):
...
def parse_input_data(data_file):
    ...
    for line in fh_data:
        ...
        if line[0] == "#":
            pass
        elif line.split()[0] == "resi":
            pass
        elif line.strip() == "":
            pass
        elif line.split()[7] == "GLY":
            pass  # we do not get order parameters for glycine and prolines
        elif line.split()[7] == "PRO":
            pass
        else:
            data = line.split()
            r1 = int(data[0])
...
if __name__ == "__main__":
    ...
    output = pd.DataFrame(
        columns=["s2calc", "s2ortho", "s2ang", "resn", "resi", "chain"]
    )
