...
### 1. Modeling alternate conformers of a ligand (X-ray)
...
### 2. Modeling alternate conformers of a ligand on an event map
...
### 3. Modeling alternate conformers of a ligand on a cryo-EM map
...
`qfit_ligand [MAP] [PDB_FILE] -r [RESOLUTION] [CHAIN,LIGAND] -sm [SMILES] -em_lig`

Using the example 8P70:

`qfit_ligand EMD-17513.map qfit_cryoem_example/8P70.pdb -r 2.0 A,201 -sm 'O=C1CCCN1NC2=NC=NC=C2C3=C(F)C=CC=N3' -em_lig`

The cryo-EM map can be downloaded from: https://www.ebi.ac.uk/emdb/EMD-17513
