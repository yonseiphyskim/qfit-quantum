# qFit-Quantum post-processing scripts
...
Ligand:
`qfit_final_refine_ligand.sh input.mtz`

Cryo-EM Ligand:
`qfit_final_refine_cryoem_ligand.sh map_file.ccp4 resolution`

## Analysis Scripts
These analysis scripts should be used on outputs after refinement. Many can be done using other popular structural programs, but we have amended many of them here to work with multiconformer models.

### 1. Calculating Order Parameters
You will first need to run a script to produce the intermediate file to calculate crystallographic order parameters.
...
You then can use the intermediate file to calculate the order parameters. Please note you will need the resolution and average alpha-carbon b-factor for normalization (see scripts below).
...
### 10. Get Root Mean Squared Fluctuations (RMSF) for each residue
...
This script will calculate the RSCC of a ligand defined by its residue number and chain. This script can either calculate the RSCC of a single model of interest, or the RSCC of two models against the same density map in the same voxel space.
